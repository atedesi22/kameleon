...
const offerings = [
  ...
  },
  {
    ...
    desc: "Un ensemble complet de templates pour votre nouvelle marque."
  },
  {
    id: 3,
    type: "Service",
    ...
    <section id="boutique" className="py-24 px-6 bg-black text-white">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-20">
          <div>
            <h2 className="text-indigo-400 font-bold uppercase tracking-widest mb-2">Shop & Services[cite: 1]</h2>
            <h3 className="text-5xl font-black">Investir dans votre vision.[cite: 1]</h3>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {offerings.map((item) => (
            <div key={item.id} className="group relative bg-zinc-900 p-10 rounded-3xl border border-zinc-800 hover:border-indigo-500 transition-all duration-500">
              ...
              <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">{item.type}[cite: 1]</span>
              <h4 className="text-2xl font-bold mt-2 mb-4">{item.title}[cite: 1]</h4>
              <p className="text-zinc-400 mb-8 leading-relaxed">{item.desc}[cite: 1]</p>
              <div className="flex items-center justify-between mt-auto">
                <span className="font-black text-xl">{item.price}[cite: 1]</span>
